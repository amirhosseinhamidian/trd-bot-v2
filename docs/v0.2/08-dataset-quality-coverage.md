...
گزارش‌های قدیمی بدون coverage همچنان با `coverage=null` خوانده می‌شوند. فیلد جدید داخل
`payload_json` ذخیره می‌شود و migration جدولی لازم ندارد. schema نسخه ۲ به‌صورت backward-compatible
حفظ شده است؛ تغییر آتی فرمول امتیاز کیفیت در TB2-006 قرارداد جداگانه خواهد داشت.
