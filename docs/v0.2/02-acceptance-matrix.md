...
| D10 | کارهای سنگین Background Job و progress داشته باشند. | foundation صف PostgreSQL دارای lifecycle، progress پایدار، idempotency، lease/recovery، retry budget، cancel و worker مستقل است. اتصال Import و Optimization هنوز باقی است. **جزئی**. | worker را میان اجرا restart کن؛ lease منقضی reclaim شود، stale worker نتواند commit کند، progress پس از reload بماند و enqueue تکراری job دوم نسازد. | TB2-008، TB2-009، TB2-013، TB2-024 |
