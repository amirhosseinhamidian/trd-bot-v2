...
| Import و فایل | `src/trd_bot/api/routes/{market_data_imports,datasets}.py`، `src/trd_bot/research/historical_dataset_imports.py`، `frontend/src/lib/datasets/csv.ts` | fetch تاریخی/preview/import/refresh/history و CSV در مرورگر موجودند. JSON/Parquet، mapping ستون و upload سروری پیدا نشد؛ ورود طولانی هنوز در request است. | TB2-004، TB2-007، TB2-009 |
...
| Jobs و Monitoring | `src/trd_bot/api/background_jobs.py`، `src/trd_bot/api/routes/monitoring.py`، `frontend/src/components/dashboard/monitoring-dashboard.tsx` | Experiment/Walk-Forward از FastAPI `BackgroundTasks` استفاده می‌کنند؛ مانیتورینگ عمومی موجود است. job بادوام، progress مشترک، import/worker health در سطح v2 تکمیل نیست. | TB2-008، TB2-009، TB2-023 |
...
| R-06 | `frontend/src/lib/datasets/csv.ts` headerهای ثابت می‌خواهد؛ `frontend/src/components/dashboard/dataset-import-form.tsx` داده را سمت مرورگر parse می‌کند؛ `src/trd_bot/api/routes/datasets.py` کندل‌ها را به صورت JSON می‌گیرد. | mapping و فایل بزرگ/فرمت‌های دیگر فاقد مسیر موردنیاز v2 هستند. | TB2-007 |
