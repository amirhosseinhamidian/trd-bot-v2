...
| Import و فایل | `src/trd_bot/api/routes/{market_data_imports,datasets}.py`، `src/trd_bot/research/{historical_dataset_imports,dataset_file_imports}.py`، `frontend/src/components/dashboard/dataset-import-form.tsx` | fetch تاریخی/preview/import/refresh/history موجود است. CSV/JSON/Parquet در Backend inspect/normalize می‌شوند، mapping صریح دارند و Import به checksum همان Preview مقید است؛ اجرای طولانی هنوز در request است. | TB2-004، TB2-007، TB2-009 |
