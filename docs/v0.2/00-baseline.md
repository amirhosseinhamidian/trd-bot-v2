...
| Strategy | `src/trd_bot/strategies/{registry,metadata}.py`، `src/trd_bot/api/routes/research.py`، `frontend/src/app/[locale]/strategies/` | کاتالوگ EMA/SMA/RSI برای هر `name@version` قرارداد پارامتر، lifecycle و fingerprint قطعی رفتار منتشر می‌کند. Experimentهای جدید fingerprint را snapshot می‌کنند و صفحهٔ Strategy به history دقیق همان نسخه می‌رسد؛ replay عملی نتیجهٔ قدیمی در مرحلهٔ بعد است. | TB2-010، TB2-011 |
