...
| D05 | Strategy Registry و version lineage در Experimentها حفظ شوند. | هر تعریف Registry lifecycle، schema و fingerprint قطعی دارد؛ Experiment fingerprint را snapshot می‌کند و صفحهٔ Strategy history همان نسخه را نشان می‌دهد. replay با Dataset/تنظیمات ثبت‌شده خروجی کامل را checksum و مقایسه می‌کند؛ legacy/missing/mismatch fail-closed و read-only هستند. **پیاده‌سازی‌شده؛ پذیرش PostgreSQL/CI باقی است**. | fingerprint نسخه و replay golden را کنترل کن؛ تغییر عمدی fingerprint و result باید mismatch، حذف Dataset و legacy باید unverifiable و رکورد اصلی باید ثابت بماند. | TB2-010، TB2-011، TB2-024 |
| D06 | Optimization با robustness و Walk-Forward قابل بازسازی باشد. | API یک grid محدود را به plan قطعی و execution queued persisted تبدیل می‌کند؛ create/list/detail واقعی‌اند و state داخلی قابل جعل نیست. اجرای durable trial، folds و robustness هنوز **ناتمام** است. | grid کوچک و grid بالای ۱۰۰ ترکیب را از API کنترل کن؛ در TB2-013 trialها persisted و retry deterministic شوند و در TB2-014 نمونه overfit بهتر رتبه نگیرد. | TB2-008، TB2-012 تا TB2-015 |
...
| D10 | کارهای سنگین Background Job و progress داشته باشند. | foundation صف PostgreSQL و worker مستقل موجود است؛ import/refresh دادهٔ Provider با enqueue اتمیک، idempotency، progress و audit هر attempt اجرا می‌شود. پردازش فایل و Optimization هنوز متصل نیستند. **جزئی**. | worker را میان Import restart کن؛ lease منقضی reclaim شود، enqueue تکراری job دوم نسازد، progress پس از reload بماند و result reference به Import audit برسد. | TB2-008، TB2-009، TB2-013، TB2-024 |
