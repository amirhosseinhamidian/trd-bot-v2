...
| Optimization | `src/trd_bot/research/{optimization,optimization_executions,optimization_jobs,optimization_worker}.py`، `src/trd_bot/api/routes/optimization_executions.py`، `alembic/versions/20260831_0014_optimization_executions.py` | API از ورودی محدود plan قطعی می‌سازد و execution/job را اتمیک و idempotent enqueue می‌کند. worker مستقل trialهای باقی‌مانده را به Experimentهای immutable تبدیل، progress را persist و نتیجهٔ برتر را ثبت می‌کند. robustness/Walk-Forward و UI هنوز مراحل بعدی‌اند. | TB2-012 تا TB2-015 |
