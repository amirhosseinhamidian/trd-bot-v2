...
| Jobs و Monitoring | `src/trd_bot/jobs.py`، `src/trd_bot/db/background_job_repositories.py`، `scripts/run_background_worker.py`، `src/trd_bot/api/routes/{jobs,monitoring}.py` | foundation صف PostgreSQL با idempotency، lease/recovery، progress، retry/cancel و API مشاهده موجود است. اتصال Import/Optimization و حذف کامل `BackgroundTasks` قدیمی هنوز مرحله‌ای است. | TB2-008، TB2-009، TB2-013، TB2-023 |
