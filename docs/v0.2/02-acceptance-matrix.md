...
| D03 | Quality Report و provenance برای Dataset قابل مشاهده باشد. | گزارش پوشش requested range شامل expected/received/missing/percent است؛ gap و head/tail ناقص reject می‌شوند و نتیجه در Dataset Detail ذخیره و نمایش داده می‌شود. امتیاز کیفیت ترکیبی همچنان به سیاست TB2-006 وابسته است. **جزئی**. | fixture کامل، head ناقص، tail ناقص و gap را در API/UI کنترل کن؛ snapshot قدیمی باید با `coverage=null` خوانده شود. | TB2-004، TB2-006 |
| D04 | Refresh نسخهٔ تازه بسازد و نسخهٔ قبلی را تغییر ندهد. | Snapshot و History موفق در یک تراکنش ثبت می‌شوند؛ stale refresh هنگام commit دوباره parent/version را کنترل و با `refresh_conflict` رد می‌شود؛ checksum مشترک Snapshot را duplicate نمی‌کند. آزمون‌های rollback، stale writer و immutability موجودند. کنترل کیفیت نسخه‌ها در TB2-006 باقی است. **جزئی**. | checksum/نتایج Experiment قدیمی پس از refresh ثابت بماند؛ failure injection هیچ Snapshot یتیمی نسازد؛ دو refresh هم‌زمان فقط یک نسخهٔ بعدی بسازند. | TB2-005، TB2-006 |
