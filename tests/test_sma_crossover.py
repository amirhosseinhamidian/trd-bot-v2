...
from trd_bot.strategies import SignalDirection, SMACrossoverStrategy
...
def create_dataset(
    ...
    return DatasetBuilder().build(
        name="SMA crossover test dataset",
        candles=candles,
    )
...
def test_strategy_generates_long_signal() -> None:
    ...
    assert signals[0].strategy_name == "sma-crossover"
...
def test_strategy_rejects_invalid_periods() -> None:
    with pytest.raises(
        ValueError,
        match="slow SMA period must be greater",
    ):
