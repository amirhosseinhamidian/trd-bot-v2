...
| Optimization | `src/trd_bot/research/{optimization,optimization_executions,optimization_runner,optimization_worker}.py`، `src/trd_bot/api/routes/optimization_executions.py`، `alembic/versions/20260831_0014_optimization_executions.py` | API تولیدی از ورودی محدود server-built plan و execution queued می‌سازد و list/detail persisted دارد. endpointهای transition داخلی در اختیار client نیستند؛ اتصال صف durable، اجرای trial، robustness/Walk-Forward و UI هنوز مراحل بعدی‌اند. | TB2-012 تا TB2-015 |
...
| --- | --- | --- | --- |
| R-01 | route تولیدی Optimization در `src/trd_bot/main.py` فعال و placeholderها با آزمون create/list/detail و failure جایگزین شده‌اند. | API و plan bounded بسته شد؛ اجرای واقعی باید در TB2-013 فقط از worker allowlisted و durable انجام شود. | بسته در TB2-012؛ worker در TB2-013 |
...
| R-07 | repository و runner Optimization هر دو از dependency request-scoped مشترک استفاده می‌کنند. | session پس از request توسط dependency generator بسته می‌شود؛ worker مرحله بعد session مستقل خود را نگه دارد. | بسته در TB2-012 |
