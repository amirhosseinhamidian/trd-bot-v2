...
| D05 | Strategy Registry و version lineage در Experimentها حفظ شوند. | هر تعریف Registry دارای lifecycle، schema پارامتر و fingerprint قطعی است؛ Experiment جدید fingerprint را snapshot می‌کند و صفحهٔ Strategy history فیلترشدهٔ همان name/version را نشان می‌دهد. payloadهای قدیمی بدون fingerprint سازگار و صریحاً legacy هستند. **lineage پیاده‌سازی‌شده؛ replay نهایی باقی است**. | fingerprint نسخه را با fixture golden کنترل کن؛ Experiment همان نسخه را از صفحه باز کن؛ در TB2-011 replay باید Dataset و fingerprint را verify و خروجی را مقایسه کند. | TB2-010، TB2-011 |
